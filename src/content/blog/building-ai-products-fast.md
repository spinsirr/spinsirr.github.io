...
description: 'The stack and patterns I use for building AI products fast, drawn from BuildLog and Fixo.'
pubDate: 'Jun 20 2026'
kind: field-note
...
These aren't abstract. BuildLog runs all of its generation on the SDK with Gemini and leans on structured output. The model hands back shaped data, not a blob you regex. Fixo's agent uses tools to create the estimate and take the payment, so the model *acts* rather than narrates. That is the line between a chatbot and a product: the model does the thing instead of describing how you might do the thing.
...
The split is the idea worth keeping: the product UI lives in Next.js, and the agent gets its own service. In Fixo that service is a Deno + Hono API that handles diagnosis, estimates, and order progress.
...
An agent loop and a web request want different things. A web request is short and stateless; an agent loop is multi-step, retries, recovers from a failed tool call, and has to stay on task across all of it. That loop is real infrastructure. It is worth building once and treating seriously instead of bolting it onto a page handler.

The honest trade-off: splitting the backend out costs you a service to run. Only pay that when the agent does real multi-step work, like Fixo. When generation is request-shaped, keep it in the app. That is why BuildLog has no separate agent service.
...
Why it's fast: you get auth and a real Postgres on day one, and typed queries that catch schema drift at compile time. That second part kills a whole category of dumb runtime bugs, including the column rename that ships and breaks something downstream three days later. Catching it in the type-checker is hours you don't lose. Both BuildLog and Fixo sit on Supabase.

The win is sharpest when an agent is involved. Drizzle gives the agent's tools, the API, and the UI one source of truth for the schema. In Fixo the agent writes estimates and orders, then a human reads them back in the product. When the tool and the UI derive their types from the same schema, the data the agent writes is the data the human sees, guaranteed by the compiler instead of by hope.

The trade-off: Supabase is an opinionated managed platform. You give up some control in exchange for not running Postgres yourself. The day you genuinely need to operate your own database, you will know. It will not be day one.

## Payments and money guardrails: Stripe

Payments show up earlier in agent products than people expect. Fixo's agent does not just chat. It writes an estimate and drives the Stripe payment. A mechanic agent that diagnoses and estimates but cannot take payment is a demo. The product is not real until money moves.

Stripe is the default for the same reason as everything else here: the least glue. Producing an estimate and requesting payment map cleanly onto a Checkout flow, with little impedance between what the agent decides and what Stripe needs.
...
A chat box is the lowest-effort agent UI, and it shows. Text scrolling in a window is fine for a prototype, but a real product needs the agent's work to land *in the interface*: an estimate appears as a card, a status flips, and a button does the thing the agent decided to do.

AG-UI is the fix: a protocol for streaming agent state and actions into the frontend, so the UI reflects what the agent is doing rather than printing what it said. In Fixo, the chat is the entry point. AG-UI is what makes the rest feel like software instead of a transcript.
...
There's a meta-version of this, and it's most of what I think about at CoreSpeed. The slowest part of building AI products is rarely the model or the UI. It is wiring every integration, identity, and failure path by hand. CoreSpeed turns that recurring work into shared infrastructure: managed connectors, built-in tools, and memory that carry across agent runtimes.
